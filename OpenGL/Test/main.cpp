...
void Circle_MidPoint(int xc, int yc, int Radius){
	...
	auto putVertex = [&](int xc, int yc, int x, int y){
...
void DrawCircle(float cx, float cy, float r, int num_segments) {
    for (int ii = 0; ii <= num_segments; ii++)   {
        float theta = 2.0f * 3.14159265359f * float(ii) / float(num_segments);
...
void Ellipse_MidPoint(int xc, int yc, int a, int b){
	...
	auto plotVertex = [&](int xc, int yc, int x, int y){
		glVertex2i(xc + x, yc + y);
		glVertex2i(xc - x, yc + y);
		glVertex2i(xc + x, yc - y);
		glVertex2i(xc - x, yc - y);
	...
	auto Round = [&](int a) { return (int)(a + 0.5);};
...
//	// LINE using method glVertex with two points are startPoint and endPoint
//	glBegin(GL_LINES);
////	dy/dx > 1
//	glVertex2i(20, 20);
//	glVertex2i(60, 200);
